import { Module } from '@nestjs/common';
import { CertificatesService } from './certificates.service';
import { CertificatesController } from './certificates.controller';
import { MulterModule } from '@nestjs/platform-express';
import { diskStorage, Options as MulterOptions } from 'multer';
import { extname } from 'path';

/**
 * Fungsi untuk menentukan direktori tujuan penyimpanan file sertifikat yang diupload.
 * @param req - Objek request
 * @param file - Objek file yang diupload
 * @param cb - Callback untuk menentukan direktori tujuan
 */
function certificateDestination(
  req: Express.Request,
  file: Express.Multer.File,
  cb: (error: Error | null, destination: string) => void,
): void {
  cb(null, './uploads/certificates/signed');
}

/**
 * Fungsi untuk membuat nama file unik untuk setiap file yang diupload.
 * Format nama: certificate-signed-{timestamp}-{random}.{ext}
 * @param req - Objek request
 * @param file - Objek file yang diupload
 * @param cb - Callback untuk menentukan nama file
 */
function certificateFilename(
  req: Express.Request,
  file: Express.Multer.File,
  cb: (error: Error | null, filename: string) => void,
): void {
  const uniqueSuffix = `${Date.now()}-${Math.round(Math.random() * 1e9)}`;
  cb(null, `certificate-signed-${uniqueSuffix}${extname(file.originalname)}`);
}

/**
 * Fungsi untuk memfilter file yang diizinkan untuk diupload.
 * Hanya file dengan ekstensi .pdf yang diperbolehkan.
 * @param req - Objek request
 * @param file - Objek file yang diupload
 * @param cb - Callback untuk menentukan apakah file diterima
 */
function pdfFileFilter(
  req: Express.Request,
  file: Express.Multer.File,
  cb: (error: Error | null, acceptFile: boolean) => void,
): void {
  if (extname(file.originalname).toLowerCase() === '.pdf') {
    cb(null, true);
  } else {
    cb(new Error('File harus PDF'), false);
  }
}

/**
 * Opsi konfigurasi Multer untuk upload file sertifikat.
 */
const multerOptions: MulterOptions = {
  storage: diskStorage({
    destination: certificateDestination,
    filename: certificateFilename,
  }),
  limits: { fileSize: 5 * 1024 * 1024 }, // Maksimal ukuran file: 5MB
  fileFilter: pdfFileFilter,
};

/**
 * Modul CertificatesModule
 *
 * Modul ini bertanggung jawab untuk mengelola fitur terkait sertifikat,
 * termasuk upload file PDF sertifikat yang sudah ditandatangani.
 */
@Module({
  imports: [MulterModule.register(multerOptions)],
  controllers: [CertificatesController],
  providers: [CertificatesService],
  exports: [CertificatesService],
})
export class CertificatesModule {}
