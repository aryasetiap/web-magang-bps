  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-lg shadow-lg p-6 w-80">
        ...
        <p className="mb-4 text-gray-700">Yakin ingin menghapus entri logbook ini?</p>
        <div className="flex justify-end gap-2">
          <button onClick={onCancel} className="px-4 py-1.5 rounded bg-gray-200 hover:bg-gray-300 text-gray-700">Batal</button>
          <button onClick={onConfirm} className="px-4 py-1.5 rounded bg-red-600 hover:bg-red-700 text-white">Hapus</button>
        </div>
      </div>
    </div>
  );
...
function Spinner() {
  return (
    <span className="inline-block w-5 h-5 border-2 border-bps-blue border-t-transparent border-solid rounded-full animate-spin align-middle" aria-label="Loading"></span>
  );
}
          ...
          <tbody className="divide-y divide-gray-100">
            {loading && logbookEntries.length === 0 ? (
              ...
              logbookEntries.map((entry) => (
                <tr
                  key={entry.id}
                  className={`bg-white hover:bg-bps-light-blue/20 transition-colors ${highlightId === entry.id ? 'bg-yellow-100 animate-pulse' : ''}`}
                >
                  ...
                  <td className="px-6 py-4 text-right text-sm font-medium flex gap-2 justify-end items-center">
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1 border border-indigo-500 text-indigo-700 hover:bg-indigo-50 px-2 py-1 rounded transition ${entry.status !== 'draft' ? 'opacity-50 cursor-not-allowed' : ''}`}
                      title="Edit logbook"
                      aria-label="Edit logbook"
                      disabled={entry.status !== 'draft' || loading}
                      tabIndex={0}
                    >
                      <PencilIcon className="h-4 w-4" /> <span className="hidden sm:inline">Edit</span>
                    </button>
                    <button
                      onClick={() => setDeleteId(entry.id)}
                      className={`flex items-center gap-1 border border-red-500 text-red-700 hover:bg-red-50 px-2 py-1 rounded transition ${entry.status !== 'draft' ? 'opacity-50 cursor-not-allowed' : ''}`}
                      title="Hapus logbook"
                      aria-label="Hapus logbook"
                      disabled={entry.status !== 'draft' || loading}
                      tabIndex={0}
                    >
                      <TrashIcon className="h-4 w-4" /> <span className="hidden sm:inline">Hapus</span>
                    </button>
                  </td>
                </tr>
              ))
