...
function AdminReportsPage() {
  ...
  const [allGraduationData, setAllGraduationData] = useState([]); // Tambahkan state ini untuk data kelulusan
  ...
  useEffect(() => {
    // Memuat semua data yang relevan dari localStorage
    ...
    const loadedGraduationData = JSON.parse(localStorage.getItem('adminGraduationData') || '[]'); // Muat data kelulusan
    setAllGraduationData(loadedGraduationData);

  }, []); // Dependensi kosong agar hanya jalan sekali saat mount

  // --- Logika Rekapitulasi & Statistik (Dijalankan setelah data dimuat) ---
  useEffect(() => {
    ...
    const finalReportGraduationStats = (() => {
      ...
      let finalReportsApproved = 0; // Disetujui
      ...
      // Gunakan allGraduationData yang sudah dimuat
    ...
    const internPerformanceRecap = allInterns.map(intern => {
      ...
      return {
        ...
        statusKelulusan: internGraduationStatus, // Gunakan status kelulusan dari data kelulusan
      };
    });

    // Simpan semua statistik ke dalam satu state
    setStats({
  ...
  }, [allInterns, allDailyActivities, allAssignments, allFinalReports, allGraduationData]); // Dependensi untuk re-calculate saat data berubah


  return (
    <div className="bg-white p-8 rounded-lg shadow-md">
      <h2 className="text-3xl font-bold text-bps-blue mb-6">Laporan & Statistik</h2>
      <p className="text-gray-700 mb-6">
        Dapatkan gambaran umum performa peserta magang dan statistik sistem.
      </p>
      ...
      {/* Rekap Performa Detail Per Peserta */}
