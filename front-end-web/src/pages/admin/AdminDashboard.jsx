...
function AdminDashboard() {
  ...
  const [stats, setStats] = useState({
    ...
    assignment: {
      ...
      totalReviewedAssignments: 0,
      totalNotSubmittedAssignments: 0,
    },
  ...
  useEffect(() => {
    const loadedApplicants = JSON.parse(
      localStorage.getItem("adminApplicants") || "[]"
    ).filter((app) => app.status === "Accepted");
    setAllInterns(loadedApplicants);

    const loadedAssignments = JSON.parse(
      localStorage.getItem("adminAssignmentsData") || "[]"
    );
    setAllAssignments(loadedAssignments);

    const loadedFinalReports = JSON.parse(
      localStorage.getItem("adminFinalReports") || "[]"
    );
    setAllFinalReports(loadedFinalReports);

    const loadedGraduationData = JSON.parse(
  ...
  useEffect(() => {
      ...
      allAssignments.forEach((assignment) => {
        ...
        Object.values(assignment.submissions).forEach((submission) => {
          ...
          if (submission.status === "Not Submitted")
            totalNotSubmittedAssignments++;
        });
      });
      return {
        totalAssignmentsGiven,
        totalSubmittedAssignments,
        totalReviewedAssignments,
        totalNotSubmittedAssignments,
      };
    })();
    ...
    const finalReportGraduationStats = (() => {
      ...
      const totalGraduatedInterns = allGraduationData.filter(
        (intern) => intern.overallGraduationStatus === "Lulus"
      ).length;

      return {
        totalFinalReportsSubmitted,
        finalReportsPending,
        finalReportsRevised,
        finalReportsApproved,
        totalGraduatedInterns,
      };
    })();

    const internPerformanceRecap = allInterns.map((intern) => {
      const internFinalReport = allFinalReports.find(
        (report) => report.internId === intern.id
      );
      const internGraduationStatus =
        allGraduationData.find((grad) => grad.id === intern.id)
  ...
  }, [allInterns, allAssignments, allFinalReports, allGraduationData]);
    ...
    <div className="bg-white p-8 rounded-lg shadow-md">
        ...
        <div className="p-5 bg-yellow-50 rounded-lg border-l-4 border-yellow-500 flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-yellow-700">
              {stats.assignment.totalReviewedAssignments} /{" "}
              {stats.assignment.totalAssignmentsGiven}
            </p>
          </div>
        </div>
      ...
      <div className="mt-8 p-6 bg-gray-50 rounded-lg border-l-4 border-gray-300">
        ...
        <ul className="list-disc list-inside text-gray-700 space-y-2">
          <li>
            Gunakan "Manajemen Akun" untuk menambahkan atau mengelola Staff dan
            Koordinator.
          </li>
          <li>
            Cek "Manajemen Pendaftar" untuk memverifikasi calon peserta magang.
          </li>
          <li>
            "Monitoring Peserta" memberikan gambaran presensi dan logbook harian
            semua peserta.
          </li>
